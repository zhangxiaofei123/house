...
-(void)setPlayerButtonGroup:(NSArray *)playerButtonGroup {
    _playerButtonGroup = playerButtonGroup;
}
...
//            }
            ...
            cell.imageView.image = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle]pathForResource:imagePath ofType:nil]];
        }
    ...
    if (_playerButtonGroup.count && itemIndex < _playerButtonGroup.count) {
   
        NSString *showB =  _playerButtonGroup[itemIndex];
        if ([showB isEqualToString:@"YES"]) {
            cell.playerButton.hidden = NO;
            cell.playerButton.tag = itemIndex;
            [cell.playerButton addTarget:self action:@selector(playerButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        }else {
...
-(void)playerButtonClick:(UIButton* )button {
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didSelectItemAtIndex:)]) {
        [self.delegate cycleScrollView:self didSelectItemAtIndex:[self pageControlIndexWithCurrentCellIndex:button.tag]];
    }
    if (self.clickItemOperationBlock) {
        self.clickItemOperationBlock([self pageControlIndexWithCurrentCellIndex:button.tag]);
    }
}
