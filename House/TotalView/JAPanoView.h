...
@property (nonatomic) CGFloat zoomFactor; // from 0 to 100; default ，value: 1，镜头大小（改变大小）
@property (nonatomic) CGFloat hAngle; // azimuth angle in radians; from 0 to 2*PI; default value: 0 镜头位置(平移位置)
@property (nonatomic) CGFloat vAngle; // elavation angle in radians; from -(PI/2) to (PI/2); default value: 0 镜头位置（平移位置）
